...
#include "pic64gx_hal_version.h"
#include "coreplex_ihc_version.h"
...
bool HSS_E51_Banner(void)
{
    ...
    mHSS_FANCY_PRINTF(LOG_STATUS,
        "PIC64GX(R) Hart Software Services (HSS) - version %d.%d.%d-" STR(VENDOR_STRING) "\n"
        "PIC64GX HAL version %d.%d.%d"
#if !IS_ENABLED(CONFIG_SKIP_DDR)
	...
	" / IHC version %d.%d.%d"
        ...
        PIC64GX_HAL_VERSION_MAJOR, PIC64GX_HAL_VERSION_MINOR, PIC64GX_HAL_VERSION_PATCH
#if !IS_ENABLED(CONFIG_SKIP_DDR)
        , DDR_DRIVER_VERSION
#endif
#if IS_ENABLED(CONFIG_USE_IHC)
