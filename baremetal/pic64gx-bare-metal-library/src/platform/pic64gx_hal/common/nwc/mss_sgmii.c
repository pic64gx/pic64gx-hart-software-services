...
uint32_t sgmii_setup(void)
{
     ...
     */
    if((BSP_SETTING_SGMII_MODE & (TX_RX_CH_EN_MASK<<TX_RX_CH_EN_OFFSET)) != 0U)
    {
...
static uint32_t sgmii_channel_setup(void)
{
    ...
    switch(sgmii_training_state)
    {
        ...
        case SGMII_TURN_ON_MACS:
            ...
            (void)mss_config_clk_rst(MSS_PERIPH_MAC0, (uint8_t) PIC64GX_HAL_FIRST_HART, PERIPHERAL_ON);
            (void)mss_config_clk_rst(MSS_PERIPH_MAC0, (uint8_t) PIC64GX_HAL_FIRST_HART, PERIPHERAL_ON);
...
static void setup_sgmii_rpc_per_config(void)
{
    CFG_DDR_SGMII_PHY->SGMII_MODE.SGMII_MODE    = (BSP_SETTING_SGMII_MODE & ~REG_CDR_MOVE_STEP);
    CFG_DDR_SGMII_PHY->CH0_CNTL.CH0_CNTL        = BSP_SETTING_CH0_CNTL;
    CFG_DDR_SGMII_PHY->CH1_CNTL.CH1_CNTL        = BSP_SETTING_CH1_CNTL;
    CFG_DDR_SGMII_PHY->RECAL_CNTL.RECAL_CNTL    = BSP_SETTING_RECAL_CNTL;
    CFG_DDR_SGMII_PHY->CLK_CNTL.CLK_CNTL        = BSP_SETTING_CLK_CNTL;
    /* ibuffmx_p and _n rx1, bit 22 and 23 , rx0, bit 20 and 21 */
    CFG_DDR_SGMII_PHY->SPARE_CNTL.SPARE_CNTL    = BSP_SETTING_SPARE_CNTL;
    CFG_DDR_SGMII_PHY->PLL_CNTL.PLL_CNTL        = BSP_SETTING_PLL_CNTL;
}
...
static void set_early_late_thresholds(uint8_t n_late_threshold, uint8_t p_early_threshold)
{
    ...
    CFG_DDR_SGMII_PHY->SPARE_CNTL.SPARE_CNTL    = (BSP_SETTING_SPARE_CNTL & N_EYE_MASK) | n_eye_values;

    /*
     * Set P values
     */
    p_eye_value = (uint32_t)(p_early_threshold << SHIFT_TO_REG_RX0_EYEWIDTH);
    CFG_DDR_SGMII_PHY->CH0_CNTL.CH0_CNTL        = ((BSP_SETTING_CH0_CNTL & REG_RX0_EYEWIDTH_P_MASK) | p_eye_value) | REG_RX0_EN_FLAG_N;
    CFG_DDR_SGMII_PHY->CH1_CNTL.CH1_CNTL        = ((BSP_SETTING_CH1_CNTL & REG_RX0_EYEWIDTH_P_MASK) | p_eye_value) | REG_RX1_EN_FLAG_N;
