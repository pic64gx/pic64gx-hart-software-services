...
/*             |  (#define BSP_SETTING_USE_CK_PUSH_DDR4_LPDDR3    1U)      */
...
/*             |  BSP_SETTING_RPC_156_VALUE 1 for 1333Mhz, 6 for 1600MHz   */
/* 0.4.011     |  ADD_CMD_CLK_MOVE_ORDER changed from 0,1,2 to 1,2,0          */
/* 0.4.010     |  BSP_SETTING_RPC_156_VALUE default changed from 1 to 6    */
...
/*             |  starting edge.                                              */
/* 0.3.029     |  BSP_SETTING_REFCLK_DDR3_1067_NUM_OFFSETS changed 2 to 1  */
...
static uint32_t rpc_156_dqdqs_init_offset = BSP_SETTING_MIN_RPC_156_VALUE;
#endif
...
#if (BSP_SETTING_USE_CK_PUSH_DDR4_LPDDR3 == 0U)
...
void pic64gx_hal_turn_ddr_selfrefresh_on(void)
{
     ...
     */
    if ((BSP_SETTING_DDRPHY_MODE & DDRPHY_MODE_RANK_MASK) ==
...
void pic64gx_hal_turn_ddr_selfrefresh_off(void)
{
	/*
	 * Turn on user setting for self refresh
	 */
...
uint32_t pic64gx_hal_ddr_selfrefresh_status(void)
{
    uint32_t status = 1U; /* self refresh on */

    if ((BSP_SETTING_DDRPHY_MODE & DDRPHY_MODE_RANK_MASK) == DDRPHY_MODE_TWO_RANKS)
    {
...
static uint32_t ddr_setup(void)
{
    ...
    ddr_type = BSP_SETTING_DDRPHY_MODE & DDRPHY_MODE_MASK;
 ...
 */
    ...
    switch (ddr_training_state)
    {
        case DDR_TRAINING_INIT:
            ...
            tip_cfg_params = BSP_SETTING_TIP_CFG_PARAMS;
            dpc_bits = BSP_SETTING_DPC_BITS ;
            write_latency = BSP_SETTING_CFG_WRITE_LATENCY_SET;
#if (TUNE_RPC_166_VALUE == 1)
            rpc_166_fifo_offset = DEFAULT_RPC_166_VALUE;
#endif
#ifdef MANUAL_ADDCMD_TRAINIG
            refclk_offset = BSP_SETTING_MAX_MANUAL_REF_CLK_PHASE_OFFSET + 1U;
#endif
            ddr_error_count = 0U;
            error = 0U;
...
/* To verify if separate reset required for DDR4 - believe it is not */
         ...
         */
            ...
            for (uint32_t d=0;d< \
                BSP_SETTING_TIP_CONFIG_PARAMS_BCLK_VCOPHS_OFFSET;d++)
            {
          ...
          case DDR_MANUAL_ADDCMD_TRAINING_SW:
                {
                    ...
                    {
#if (BSP_SETTING_USE_CK_PUSH_DDR4_LPDDR3 == 1U)
                        address_cmd_training_with_ck_push(ddr_type,\
                            &refclk_sweep_index, retry_count, &bclk_phase,\
                                &bclk90_phase, &refclk_phase, &refclk_offset );
#else
        ...
        case DDR_TRAINING_IP_SM_START:
            {
                CFG_DDR_SGMII_PHY->training_skip.training_skip      =\
                                        BSP_SETTING_TRAINING_SKIP_SETTING;
        ...
        case DDR_TRAINING_IP_SM_START_CHECK:
...
#endif
            {
                ...
                if(BSP_SETTING_TRAINING_SKIP_SETTING & BCLK_SCLK_BIT)
                {
        ...
        case DDR_TRAINING_IP_SM_ADDCMD:
            if(BSP_SETTING_TRAINING_SKIP_SETTING & ADDCMD_BIT)
            {
                timeout = 0xFFFFF;
        ...
        case DDR_TRAINING_IP_SM_WRLVL:
            /* END VREFTRN */
            if(BSP_SETTING_TRAINING_SKIP_SETTING & WRLVL_BIT)
            {
                timeout = 0xFFFF;
        ...
        case DDR_TRAINING_IP_SM_RDGATE:
            ...
            CFG_DDR_SGMII_PHY->rpc3_ODT.rpc3_ODT = BSP_SETTING_RPC_ODT_DQ;
            /* end addition 11th Feb 22 */
            if(BSP_SETTING_TRAINING_SKIP_SETTING & RDGATE_BIT)
            {
                timeout = 0xFFFF;
        ...
        case DDR_TRAINING_IP_SM_DQ_DQS:
            if(BSP_SETTING_TRAINING_SKIP_SETTING & DQ_DQS_BIT)
            {
                timeout = 0xFFFF;
        ...
        case DDR_TRAINING_IP_SM_VERIFY:
            if ((DDRCFG->DFI.STAT_DFI_TRAINING_COMPLETE.STAT_DFI_TRAINING_COMPLETE & 0x01U) == 0x01U)
            {
                 ...
                 for (lane_sel=0U; lane_sel< \
                            BSP_SETTING_DATA_LANES_USED; lane_sel++)
                 {
                     delay(10U);
                      ...
                      * */
                     {
                        ...
                        if((BSP_SETTING_TRAINING_SKIP_SETTING & ADDCMD_BIT) != ADDCMD_BIT)
                        {
                                                                        ...
                                                                        DQ_DQS_NUM_TAPS)
                     {
                          ...
                          */
#ifdef TUNE_RPC_156_DQDQS_INIT_VALUE
                         if (rpc_156_dqdqs_init_offset <= BSP_SETTING_MAX_RPC_156_VALUE)
                         {
...
#define DCT_EXTRA_CHECKS
        ...
        case DDR_TRAINING_WRITE_CALIBRATION:
            ...
            if(error == 0U)
            {
                if((ddr_type == DDR3)||(ddr_type == DDR3L)) /* Changing WPU and WPD */
                {
                    ...
                    if (BSP_SETTING_DDRPHY_MODE & DDRPHY_MODE_ECC_MASK)
                    {
        ...
        case DDR_FULL_32BIT_NC_CHECK:
             ...
             */
            {
                ...
                error = ddr_read_write_fn((uint64_t*)BSP_SETTING_DDR_32_NON_CACHE,\
        ...
        case DDR_FULL_32BIT_CACHE_CHECK:
            ...
            error = ddr_read_write_fn((uint64_t*)BSP_SETTING_DDR_32_CACHE,\
                                    SW_CFG_NUM_READS_WRITES,\
                                    SW_CONFIG_PATTERN);
#endif
            if(error == 0U)
        ...
        case DDR_LOAD_PATTERN_TO_CACHE_SETUP:
            pattern_test.base = BSP_SETTING_DDR_32_CACHE;
        ...
        case DDR_TRAINING_INIT_ALL_MEMORY:
#ifdef DEBUG_DDR_INIT
            mem_size = BSP_SETTING_CFG_AXI_END_ADDRESS_AXI2_1 +\
                (BSP_SETTING_CFG_AXI_END_ADDRESS_AXI2_0 + 1U);
                    ...
                    DDRPHY_MODE_ECC_ON)
            {
                mem_size = BSP_SETTING_CFG_AXI_END_ADDRESS_AXI2_1 +\
                        (BSP_SETTING_CFG_AXI_END_ADDRESS_AXI2_0 + 1U);
                pattern_test.base = BSP_SETTING_DDR_64_NON_CACHE;
            ...
            mem_size = BSP_SETTING_CFG_AXI_END_ADDRESS_AXI2_1 +\
                    (BSP_SETTING_CFG_AXI_END_ADDRESS_AXI2_0 + 1U);
            load_ddr_pattern(BSP_SETTING_DDR_64_NON_CACHE, mem_size,\
                    DDR_INIT_FILL, 0U);
#endif
        ...
        case DDR_TRAINING_FINISH_CHECK:
            ...
            ddr_diag.train_time = (uint64_t)(rdcycle() - training_start_cycle)\
                / (BSP_SETTING_MSS_COREPLEX_CPU_CLK/1000);
#ifdef DEBUG_DDR_INIT
            (void)uprint32(g_debug_uart, "\n\r ddr train time (ms): ",\
...
static uint8_t get_num_lanes(void)
{
    ...
    if ((BSP_SETTING_DDRPHY_MODE & DDRPHY_MODE_BUS_WIDTH_MASK) ==\
...
static void set_ddr_mode_reg_and_vs_bits(uint32_t dpc_bits)
{
    DDR_TYPE ddr_type = BSP_SETTING_DDRPHY_MODE & DDRPHY_MODE_MASK;
    /*
     * R1.6
    ...
    */
    {
        if ((ddr_type == DDR4) &&\
                (BSP_SETTING_DDRPHY_MODE & DDRPHY_MODE_ECC_MASK) ==\
                    DDRPHY_MODE_ECC_ON)
        {
            /*
             ...
             */
            CFG_DDR_SGMII_PHY->DDRPHY_MODE.DDRPHY_MODE  =\
                             (BSP_SETTING_DDRPHY_MODE  & DMI_DBI_MASK );
        }
        else
        {
            CFG_DDR_SGMII_PHY->DDRPHY_MODE.DDRPHY_MODE  =\
...
static void set_ddr_rpc_regs(DDR_TYPE ddr_type)
{
            ...
            case LPDDR4:
                {
...
#ifndef BSP_SETTING_RPC_EN_ADDCMD0_OVRT9
                    /*
                    ...
                    CFG_DDR_SGMII_PHY->ovrt10.ovrt10 =\
                            BSP_SETTING_RPC_EN_ADDCMD1_OVRT10;
                    {
                    ...
                    CFG_DDR_SGMII_PHY->ovrt11.ovrt11 =\
                            BSP_SETTING_RPC_EN_ADDCMD2_OVRT11;
#endif
                    /* Required when rank x 2 */
                    if ((BSP_SETTING_DDRPHY_MODE & DDRPHY_MODE_RANK_MASK) ==\
                            DDRPHY_MODE_TWO_RANKS)
...
#else
    CFG_DDR_SGMII_PHY->rpc156.rpc156 = BSP_SETTING_RPC_156_VALUE;
#endif
...

    {
         ...
         */
        CFG_DDR_SGMII_PHY->rpc1_ODT.rpc1_ODT = BSP_SETTING_RPC_ODT_ADDCMD;
        CFG_DDR_SGMII_PHY->rpc2_ODT.rpc2_ODT = BSP_SETTING_RPC_ODT_CLK;
        CFG_DDR_SGMII_PHY->rpc3_ODT.rpc3_ODT = BSP_SETTING_RPC_ODT_DQ;
        CFG_DDR_SGMII_PHY->rpc4_ODT.rpc4_ODT = BSP_SETTING_RPC_ODT_DQS;
    }
    {
        /*
...
static void ddr_off_mode(void)
{
    /*
     ...
     CFG_DDR_SGMII_PHY->DDRPHY_MODE.DDRPHY_MODE  =\
             (BSP_SETTING_DDRPHY_MODE_OFF /* & DMI_DBI_MASK */);
     /*
      * VS for off mode
      */
     CFG_DDR_SGMII_PHY->DPC_BITS.DPC_BITS        =\
             BSP_SETTING_DPC_BITS_OFF_MODE;
...
void setup_ddr_segments(SEG_SETUP option)
{
    ...
    {
        SEG[0].u[0].raw = (BSP_SETTING_SEG0_0 & 0x7FFFUL);
        SEG[0].u[1].raw = (BSP_SETTING_SEG0_1 & 0x7FFFUL);
        SEG[1].u[2].raw = (BSP_SETTING_SEG1_2 & 0x7FFFUL);
        SEG[1].u[3].raw = (BSP_SETTING_SEG1_3 & 0x7FFFUL);
        SEG[1].u[4].raw = (BSP_SETTING_SEG1_4 & 0x7FFFUL);
        SEG[1].u[5].raw = (BSP_SETTING_SEG1_5 & 0x7FFFUL);
    }
    /*
     * disable ddr blocker
...
static uint8_t bclk_sclk_offset(DDR_TYPE ddr_type)
{
    ...
    switch (ddr_type)
    {
        default:
        case DDR_OFF_MODE:
            result = BSP_SETTING_SW_TRAING_BCLK_SCLK_OFFSET_LPDDR4;
            break;
        case DDR3L:
            result = BSP_SETTING_SW_TRAING_BCLK_SCLK_OFFSET_DDR3L;
            break;
        case DDR3:
            result = BSP_SETTING_SW_TRAING_BCLK_SCLK_OFFSET_DDR3;
            break;
        case DDR4:
            result = BSP_SETTING_SW_TRAING_BCLK_SCLK_OFFSET_DDR4;
            break;
        case LPDDR3:
            result = BSP_SETTING_SW_TRAING_BCLK_SCLK_OFFSET_LPDDR3;
            break;
        case LPDDR4:
            result = BSP_SETTING_SW_TRAING_BCLK_SCLK_OFFSET_LPDDR4;
            break;
 ...
 *
 * Note: If BSP_SETTING_RPC_EN_ADDCMD0_OVRT9 is not present, indicates older
 * Libero core (pre 2.0.109)
...
static void config_ddr_io_pull_up_downs_rpc_bits(DDR_TYPE ddr_type)
{
    ...
    {
#ifdef BSP_SETTING_RPC_EN_ADDCMD0_OVRT9
        /* set over-rides (associated bit set to 1 if I/O not being used */
        CFG_DDR_SGMII_PHY->ovrt9.ovrt9   = BSP_SETTING_RPC_EN_ADDCMD0_OVRT9;
        CFG_DDR_SGMII_PHY->ovrt10.ovrt10 = BSP_SETTING_RPC_EN_ADDCMD1_OVRT10;
        CFG_DDR_SGMII_PHY->ovrt11.ovrt11 = BSP_SETTING_RPC_EN_ADDCMD2_OVRT11;
        CFG_DDR_SGMII_PHY->ovrt12.ovrt12 = BSP_SETTING_RPC_EN_DATA0_OVRT12;
        CFG_DDR_SGMII_PHY->ovrt13.ovrt13 = BSP_SETTING_RPC_EN_DATA1_OVRT13;
        CFG_DDR_SGMII_PHY->ovrt14.ovrt14 = BSP_SETTING_RPC_EN_DATA2_OVRT14;
        CFG_DDR_SGMII_PHY->ovrt15.ovrt15 = BSP_SETTING_RPC_EN_DATA3_OVRT15;
        CFG_DDR_SGMII_PHY->ovrt16.ovrt16 = BSP_SETTING_RPC_EN_ECC_OVRT16;
        /* set the required wpu state- note: associated I/O bit 1=> off, 0=> on */
        CFG_DDR_SGMII_PHY->rpc235.rpc235 = BSP_SETTING_RPC235_WPD_ADD_CMD0;
        CFG_DDR_SGMII_PHY->rpc236.rpc236 = BSP_SETTING_RPC236_WPD_ADD_CMD1;
        CFG_DDR_SGMII_PHY->rpc237.rpc237 = BSP_SETTING_RPC237_WPD_ADD_CMD2;
        CFG_DDR_SGMII_PHY->rpc238.rpc238 = BSP_SETTING_RPC238_WPD_DATA0;
        CFG_DDR_SGMII_PHY->rpc239.rpc239 = BSP_SETTING_RPC239_WPD_DATA1;
        CFG_DDR_SGMII_PHY->rpc240.rpc240 = BSP_SETTING_RPC240_WPD_DATA2;
        CFG_DDR_SGMII_PHY->rpc241.rpc241 = BSP_SETTING_RPC241_WPD_DATA3;
        CFG_DDR_SGMII_PHY->rpc242.rpc242 = BSP_SETTING_RPC242_WPD_ECC;
        /* set the required wpd state- note: associated I/O bit 1=> off, 0=> on */
        CFG_DDR_SGMII_PHY->rpc243.rpc243 = BSP_SETTING_RPC243_WPU_ADD_CMD0;
        CFG_DDR_SGMII_PHY->rpc244.rpc244 = BSP_SETTING_RPC244_WPU_ADD_CMD1;
        CFG_DDR_SGMII_PHY->rpc245.rpc245 = BSP_SETTING_RPC245_WPU_ADD_CMD2;
        CFG_DDR_SGMII_PHY->rpc246.rpc246 = BSP_SETTING_RPC246_WPU_DATA0;
        CFG_DDR_SGMII_PHY->rpc247.rpc247 = BSP_SETTING_RPC247_WPU_DATA1;
        CFG_DDR_SGMII_PHY->rpc248.rpc248 = BSP_SETTING_RPC248_WPU_DATA2;
        CFG_DDR_SGMII_PHY->rpc249.rpc249 = BSP_SETTING_RPC249_WPU_DATA3;
        CFG_DDR_SGMII_PHY->rpc250.rpc250 = BSP_SETTING_RPC250_WPU_ECC;
#endif
    }
}
...
const uint8_t REFCLK_OFFSETS[][5U] = {
        {BSP_SETTING_REFCLK_DDR3_1333_NUM_OFFSETS,
        BSP_SETTING_REFCLK_DDR3_1333_OFFSET_0,
        BSP_SETTING_REFCLK_DDR3_1333_OFFSET_1,
        BSP_SETTING_REFCLK_DDR3_1333_OFFSET_2,
        BSP_SETTING_REFCLK_DDR3_1333_OFFSET_3},
        {
        BSP_SETTING_REFCLK_DDR3L_1333_NUM_OFFSETS,
        BSP_SETTING_REFCLK_DDR3L_1333_OFFSET_0,
        BSP_SETTING_REFCLK_DDR3L_1333_OFFSET_1,
        BSP_SETTING_REFCLK_DDR3L_1333_OFFSET_2,
        BSP_SETTING_REFCLK_DDR3L_1333_OFFSET_3},
        {
        BSP_SETTING_REFCLK_DDR4_1600_NUM_OFFSETS,
        BSP_SETTING_REFCLK_DDR4_1600_OFFSET_0,
        BSP_SETTING_REFCLK_DDR4_1600_OFFSET_1,
        BSP_SETTING_REFCLK_DDR4_1600_OFFSET_2,
        BSP_SETTING_REFCLK_DDR4_1600_OFFSET_3},
        {
        BSP_SETTING_REFCLK_LPDDR3_1600_NUM_OFFSETS,
        BSP_SETTING_REFCLK_LPDDR3_1600_OFFSET_0,
        BSP_SETTING_REFCLK_LPDDR3_1600_OFFSET_1,
        BSP_SETTING_REFCLK_LPDDR3_1600_OFFSET_2,
        BSP_SETTING_REFCLK_LPDDR3_1600_OFFSET_3},
        {
        BSP_SETTING_REFCLK_LPDDR4_1600_NUM_OFFSETS,
        BSP_SETTING_REFCLK_LPDDR4_1600_OFFSET_0,
        BSP_SETTING_REFCLK_LPDDR4_1600_OFFSET_1,
        BSP_SETTING_REFCLK_LPDDR4_1600_OFFSET_2,
        BSP_SETTING_REFCLK_LPDDR4_1600_OFFSET_3},

        {BSP_SETTING_REFCLK_DDR3_1067_NUM_OFFSETS,
        BSP_SETTING_REFCLK_DDR3_1067_OFFSET_0,
        BSP_SETTING_REFCLK_DDR3_1067_OFFSET_1,
        BSP_SETTING_REFCLK_DDR3_1067_OFFSET_2,
        BSP_SETTING_REFCLK_DDR3_1067_OFFSET_3},
        {
        BSP_SETTING_REFCLK_DDR3L_1067_NUM_OFFSETS,
        BSP_SETTING_REFCLK_DDR3L_1067_OFFSET_0,
        BSP_SETTING_REFCLK_DDR3L_1067_OFFSET_1,
        BSP_SETTING_REFCLK_DDR3L_1067_OFFSET_2,
        BSP_SETTING_REFCLK_DDR3L_1067_OFFSET_3},
        {
        BSP_SETTING_REFCLK_DDR4_1333_NUM_OFFSETS,
        BSP_SETTING_REFCLK_DDR4_1333_OFFSET_0,
        BSP_SETTING_REFCLK_DDR4_1333_OFFSET_1,
        BSP_SETTING_REFCLK_DDR4_1333_OFFSET_2,
        BSP_SETTING_REFCLK_DDR4_1333_OFFSET_3},
        {
        BSP_SETTING_REFCLK_LPDDR3_1333_NUM_OFFSETS,
        BSP_SETTING_REFCLK_LPDDR3_1333_OFFSET_0,
        BSP_SETTING_REFCLK_LPDDR3_1333_OFFSET_1,
        BSP_SETTING_REFCLK_LPDDR3_1333_OFFSET_2,
        BSP_SETTING_REFCLK_LPDDR3_1333_OFFSET_3},
        {
        BSP_SETTING_REFCLK_LPDDR4_1333_NUM_OFFSETS,
        BSP_SETTING_REFCLK_LPDDR4_1333_OFFSET_0,
        BSP_SETTING_REFCLK_LPDDR4_1333_OFFSET_1,
        BSP_SETTING_REFCLK_LPDDR4_1333_OFFSET_2,
        BSP_SETTING_REFCLK_LPDDR4_1333_OFFSET_3},
};
...
static uint8_t ddr_manual_addcmd_refclk_offset(DDR_TYPE ddr_type, uint8_t * refclk_sweep_index)
{
    ...
    switch (ddr_type)
    {
        case DDR3L:
        case DDR3:
            if(BSP_SETTING_DDR_CLK + DDR_FREQ_MARGIN < DDR_1333_MHZ)
            {
        ...
        case LPDDR3:
        case LPDDR4:
            if(BSP_SETTING_DDR_CLK + DDR_FREQ_MARGIN < DDR_1600_MHZ)
            {
                type_array_index = (uint8_t)(type_array_index + (uint8_t)LPDDR4 + (uint8_t)1U);
            }
            break;
...
uint32_t add_cmd_move_order_0_deg[]={\
        BSP_SETTING_ADD_CMD_CLK_MOVE_ORDER_0_DEG_DDR3,\
        BSP_SETTING_ADD_CMD_CLK_MOVE_ORDER_0_DEG_DDR3L,\
        BSP_SETTING_ADD_CMD_CLK_MOVE_ORDER_0_DEG_DDR4,\
        BSP_SETTING_ADD_CMD_CLK_MOVE_ORDER_0_DEG_LPDDR3,\
        BSP_SETTING_ADD_CMD_CLK_MOVE_ORDER_0_DEG_LPDDR4 };

uint32_t add_cmd_move_order_45_deg[]={\
        BSP_SETTING_ADD_CMD_CLK_MOVE_ORDER_45_DEG_DDR3,\
        BSP_SETTING_ADD_CMD_CLK_MOVE_ORDER_45_DEG_DDR3L,\
        BSP_SETTING_ADD_CMD_CLK_MOVE_ORDER_45_DEG_DDR4,\
        BSP_SETTING_ADD_CMD_CLK_MOVE_ORDER_45_DEG_LPDDR3,\
        BSP_SETTING_ADD_CMD_CLK_MOVE_ORDER_45_DEG_LPDDR4 };

uint32_t add_cmd_move_order_90_deg[]={\
        BSP_SETTING_ADD_CMD_CLK_MOVE_ORDER_90_DEG_DDR3,\
        BSP_SETTING_ADD_CMD_CLK_MOVE_ORDER_90_DEG_DDR3L,\
        BSP_SETTING_ADD_CMD_CLK_MOVE_ORDER_90_DEG_DDR4,\
        BSP_SETTING_ADD_CMD_CLK_MOVE_ORDER_90_DEG_LPDDR3,\
        BSP_SETTING_ADD_CMD_CLK_MOVE_ORDER_90_DEG_LPDDR4 };
