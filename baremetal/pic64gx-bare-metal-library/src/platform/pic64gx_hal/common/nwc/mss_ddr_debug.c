...
uint32_t tip_register_status (mss_uart_instance_t *g_mss_uart_debug_pt)
{
    ...
    for (rank_sel=0; rank_sel < BSP_SETTING_CFG_NUM_RANKS; rank_sel++)
    {
        ...
        for (ddr_lane_sel=0U; ddr_lane_sel < BSP_SETTING_DATA_LANES_USED; ddr_lane_sel++)
        {
...
uint32_t test_ddr(uint32_t no_of_iterations, volatile PATTERN_TEST_PARAMS *pattern_test)
{
    ...
    while(iteration < no_of_iterations)
    {
        ...
        if (((uint64_t)p_ddr_cached + ( 2 * pattern_length)) <  (BSP_SETTING_DDR_32_CACHE + pattern_test->size))
        {
