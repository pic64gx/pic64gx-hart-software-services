 ...
 * @brief PIC64GX MSS functions related to peripherals.
 *
 */
...
const uint32_t BSP_SETTING_CONTEXT_EN[][2U] = {
                {BSP_SETTING_CONTEXT_A_EN,
                 BSP_SETTING_CONTEXT_B_EN},
                {BSP_SETTING_CONTEXT_A_EN_FIC,
                 BSP_SETTING_CONTEXT_B_EN_FIC},
};
...
static inline uint8_t verify_context_enable(uint8_t option, uint32_t periph_context_mask , uint8_t hart)
{
    ...
    if (hart != (uint8_t) 0U)
    {
        if (BSP_SETTING_CONTEXT_A_HART_EN & hart )
        {
            if (BSP_SETTING_CONTEXT_EN[option][0U] & periph_context_mask)
            {
                result = 0U;
            }
        }

        if (BSP_SETTING_CONTEXT_B_HART_EN & hart )
        {
            if (BSP_SETTING_CONTEXT_EN[option][1U] & periph_context_mask)
            {
                result = 0U;
            }
        }
 ...
 * See mss_peripherals.h for details of how to use this function.
 */
__attribute__((weak)) void mss_enable_coreplex(void)
{
    /* Remove soft reset */
    SYSREG->SOFT_RESET_CR &= (uint32_t)~(SOFT_RESET_CR_COREPLEX_MASK);
}

/***************************************************************************//**
 * See mss_peripherals.h for details of how to use this function.
 */
__attribute__((weak)) void mss_disable_coreplex(void)
{
    /* Apply reset */
    SYSREG->SOFT_RESET_CR |= SOFT_RESET_CR_COREPLEX_MASK;
}
