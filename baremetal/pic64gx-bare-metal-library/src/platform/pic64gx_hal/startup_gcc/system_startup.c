...
/*==============================================================================
 * This function is called by the lowest enabled hart (PIC64GX_HAL_FIRST_HART) in
 * the configuration file :
 * (src/boards/<my-board/platform_config/pic64gx_hal_config/mss_sw_config.h )
 *
 * The other harts up to PIC64GX_HAL_LAST_HART are placed in wfi at this point.
...
__attribute__((weak)) int main_first_hart(HLS_DATA* hls)
{
    ...
    if(hartid == PIC64GX_HAL_FIRST_HART)
    {
        uint8_t hart_id;
        ptrdiff_t stack_top;
         ...
         * Please comment/uncomment PIC64GX_HAL_HW_CONFIG define in
         * src/boards/<my-board/platform_config/pic64gx_hal_config/sw_config.h
         * as required.
         */
        init_memory();
#ifndef PIC64GX_HAL_HW_CONFIG
        hls->my_hart_id = PIC64GX_HAL_FIRST_HART;
#endif
#ifdef  PIC64GX_HAL_HW_CONFIG
        load_virtual_rom();
        (void)init_bus_error_unit();
        (void)init_mem_protection_unit();
        (void)init_pmp((uint8_t)PIC64GX_HAL_FIRST_HART);
        (void)mss_set_apb_bus_cr((uint32_t)BSP_SETTING_APBBUS_CR);
        (void)mss_set_gpio_interrupt_fab_cr((uint32_t)BSP_SETTING_GPIO_INTERRUPT_FAB_CR);
        ...
        */
        ...
        hart_id = PIC64GX_HAL_FIRST_HART + 1U;
        while( hart_id <= PIC64GX_HAL_LAST_HART)
        {
         ...
         */
        (void)mss_config_clk_rst(MSS_PERIPH_FIC0, (uint8_t)PIC64GX_HAL_FIRST_HART, PERIPHERAL_ON);
        (void)mss_config_clk_rst(MSS_PERIPH_FIC1, (uint8_t)PIC64GX_HAL_FIRST_HART, PERIPHERAL_ON);
        (void)mss_config_clk_rst(MSS_PERIPH_FIC2, (uint8_t)PIC64GX_HAL_FIRST_HART, PERIPHERAL_ON);
        (void)mss_config_clk_rst(MSS_PERIPH_FIC3, (uint8_t)PIC64GX_HAL_FIRST_HART, PERIPHERAL_ON);
        /* enable the fabric */
        mss_enable_coreplex();
 ...
 * space.
 *
 * Please note: harts PIC64GX_HAL_FIRST_FIRST + 1 to PIC64GX_HAL_FIRST_LAST will wait
 * in startup code in entry.S as they run the wfi (wait for interrupt)
 * instruction.
 * They are woken up as required by the the PIC64GX_HAL_FIRST_HART, in the function
 * main_first_hart().
...
#ifdef  PIC64GX_HAL_HW_CONFIG
    turn_on_fpu((uint32_t)BSP_SETTING_TURN_ON_FPU);
#ifdef  PIC64GX_HAL_SHARED_MEM_ENABLED
    /*
     * If we are a boot-loader, and shared memory enabled (PIC64GX_HAL_SHARED_MEM_ENABLED)
...
__attribute__((weak)) void u54_4(void)
{
 ...
 /*-----------------------------------------------------------------------------
  ...
  {
    if ((BSP_SETTING_DDRPHY_MODE & DDRPHY_MODE_MASK) != DDR_OFF_MODE) {
#ifdef DDR_SUPPORT
        uint64_t end_address;

#if 0 /* enable to init cache to zero using 64 bit writes */
        end_address = BSP_SETTING_DDR_64_NON_CACHE + BSP_SETTING_CFG_AXI_END_ADDRESS_AXI2_0 + BSP_SETTING_CFG_AXI_END_ADDRESS_AXI2_1;
        zero_section((uint64_t *)BSP_SETTING_DDR_64_NON_CACHE, (uint64_t *)end_address);
#endif

        end_address = BSP_SETTING_DDR_64_CACHE + BSP_SETTING_CFG_AXI_END_ADDRESS_AXI1_0 + BSP_SETTING_CFG_AXI_END_ADDRESS_AXI1_1;
        zero_section((uint64_t *)BSP_SETTING_DDR_64_CACHE, (uint64_t *)end_address);
#endif
    }
  }
...
{
    uint32_t hart_id = PIC64GX_HAL_FIRST_HART;
