...
struct IPI_Data {
    ...
    } pic64gx_ipi_privateData[MAX_NUM_HARTS];
};
...
bool CLINT_Raise_MSIP(enum HSSHartId const target)
{
    ...
    if (result) {
        IPI_DATA.pic64gx_ipi_privateData[target].ipi_sends++;
    }
...
bool IPI_ConsumeIntent(enum HSSHartId source, enum IPIMessagesEnum msg_type)
{
...

    {
        ...
        // assert that it is the correct handler type...
        {
            ...
            if (msg_type != ipiRegistry[msg_type].msg_type) {
                mHSS_DEBUG_PRINTF(LOG_ERROR, "%u vs %u\n", msg_type, ipiRegistry[msg_type].msg_type);
            }
        ...
        if (pHandler && intentFound) {
            ...
            IPI_DATA.pic64gx_ipi_privateData[current_hartid()].consume_intents++;
...
bool IPI_MessageAlloc(uint32_t *indexOut)
{
    ...
    if (result) {
        IPI_DATA.pic64gx_ipi_privateData[current_hartid()].message_allocs++;
    }
                 ...
                 void const *p_ancilliary_buffer_in_ddr)
{
    ...
    if (result) {
        IPI_DATA.pic64gx_ipi_privateData[current_hartid()].message_delivers++;
    }
...
void IPI_MessageFree(uint32_t index)
{
    ...
    IPI_DATA.pic64gx_ipi_privateData[current_hartid()].message_frees++;
...
void IPI_DebugDumpStats(void)
{
    ...
    for (myHartId = HSS_HART_E51; myHartId < HSS_HART_NUM_PEERS; myHartId++) {
        ...
        mHSS_DEBUG_PRINTF(LOG_STATUS, "message_allocs:   %" PRIu64 "\n",
            IPI_DATA.pic64gx_ipi_privateData[myHartId].message_allocs);
        mHSS_DEBUG_PRINTF(LOG_STATUS, "message_delivers: %" PRIu64 "\n",
            IPI_DATA.pic64gx_ipi_privateData[myHartId].message_delivers);
        mHSS_DEBUG_PRINTF(LOG_STATUS, "message_frees:    %" PRIu64 "\n",
            IPI_DATA.pic64gx_ipi_privateData[myHartId].message_allocs);
        mHSS_DEBUG_PRINTF(LOG_STATUS, "consume_intents:  %" PRIu64 "\n",
            IPI_DATA.pic64gx_ipi_privateData[myHartId].consume_intents);
        mHSS_DEBUG_PRINTF(LOG_STATUS, "ipi_sends:        %" PRIu64 "\n\n",
            IPI_DATA.pic64gx_ipi_privateData[myHartId].ipi_sends);
    }
#endif
