...
extern const struct sbi_hsm_device pic64gx_hsm;
...
static void opensbi_scratch_setup(enum HSSHartId hartid)
{
    ...
    sbi_hsm_set_device(&pic64gx_hsm);
}
...
//
#define PIC64GX_HSS_SBI_IMPID	8

static void opensbi_init_handler(struct StateMachine * const pMyMachine)
{
    pMyMachine->state++;

    sbi_ecall_set_impid(PIC64GX_HSS_SBI_IMPID);
}
...
void __noreturn HSS_OpenSBI_DoBoot(enum HSSHartId hartid)
{
    ...
    opensbi_scratch_setup(hartid);
    pic64gx_mark_hart_as_booted(hartid);
    ...
    if (source != HSS_HART_E51) { // prohibited by policy
        ...
        if (result != IPI_FAIL) {
            ...
            if (p_ancilliary_buffer_in_ddr) {
                ...
                extern unsigned long _binary_build_services_opensbi_pic64gx_dtb_start;
                scratches[hartid].scratch.next_arg1 = (unsigned long)&_binary_build_services_opensbi_pic64gx_dtb_start;
#else
